...
<html style="font-size: 16px;">
  <head>
    ...
    <title>Thống kê phiếu nhập</title>
...
<!-- Kết thúc phần header -->
      ...
      <div class="u-clearfix u-sheet u-sheet-1">
        <h2 class="u-text u-text-1">THỐNG KÊ PHIẾU NHẬP</h2>
            ...
            <div class="u-align-center u-form-group u-form-submit">
              ...
              <?php
               ...
               if(isset($_POST['Chon']))
               {
                ...
                $ThongKe = $conn -> query("SELECT COUNT(pn.PN_MaPN) 
                                           FROM phieunhap pn, chitietphieunhap ct
                                           WHERE pn.PN_MaPN = ct.CTN_MaPN
                                                 AND (pn.PN_Ngaylap BETWEEN '$ngayBD' AND '$ngayKT')");
                $row = $ThongKe -> fetch_assoc();
                $SoPN = $row['COUNT(pn.PN_MaPN)'];
                if($SoPN == 0)
                {
                  echo "Không có phiếu nhập trong khoảng thời gian này";
                }
                else
                {
                  echo "Tổng số phiếu nhập kho từ ngày $newDate1 đến $newDate2 là $SoPN phiếu";
                }
               }
               ?>
